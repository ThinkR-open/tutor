...
Ce module utilise des fonctions [du package {tidyverse}](http://tidyverse.org/){target="_blank}, notamment des fonctions de {ggplot2}, {tibble} et {dplyr}, ainsi que le package `{prenoms}`. Tous ces packages ont été préinstallés et préchargés.
...
Revenons à notre question : quels sont les prénoms les plus populaires de tous les temps ?
...
Tout comme `select()`, `filter()` et `arrange()`, ces fonctions prennent toutes un jeu de données comme premier argument et renvoient un nouveau jeu de données comme sortie, et nous utiliserons le pipe ( |> ) pour les articuler.
...
Nous avons utilisé `summarise()` ci-dessus pour calculer le nombre total de filles prénommés "Margot", mais complétons ce code pour calculer également :

* `max` - le nombre maximum de filles prénommés "Margot" en une seule année
* `mean` - le nombre moyen de filles prénommés "Margot" par an
...
`group_by()` prend un jeu de données puis les noms d'une ou plusieurs colonnes de ce jeu de données. Il renvoie une copie du jeu de données qui a été "groupé" en ensemble de lignes qui partagent des combinaisons identiques de valeurs dans les colonnes spécifiées.
...
Dans le `summarise()`, `.groups = "drop"` indique de ne pas conserver les groupes après le résumé ({dplyr} > 1.0).
Si vous souhaitez supprimer manuellement les critères de regroupement d'un jeu de données après toute autre  fonction que `summarise()`, vous pouvez le faire avec `ungroup()`.
...
prenoms |> 
  ...
  mutate(rank = min_rank(desc(prop))) |> 
  group_by(name, sex) |> 
  summarise(score = median(rank), .groups = "drop") |> 
  arrange(score)
```
...
prenoms |> 
  group_by(year, sex) |> 
  summarise(n_names = n_distinct(name), .groups = "drop") |> # or summarise(n_names = n(), .groups = "drop")
  ggplot() +
...
prenoms |> 
  group_by(year, sex) |> 
  summarise(per_name = mean(n), .groups = "drop") |> 
  ggplot() +
