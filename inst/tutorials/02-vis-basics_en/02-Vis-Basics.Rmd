...
In this tutorial, we will use the [core tidyverse packages](http://tidyverse.org/){target="_blank}, including `ggplot2`. I've already loaded the packages for you, so let's begin! 

***

These examples are excerpted from _R for Data Science_ by Hadley Wickham and Garrett Grolemund, published by O’Reilly Media, Inc., 2016, ISBN: 9781491910399. You can purchase the book at [shop.oreilly.com](http://shop.oreilly.com/product/0636920034407.do){target="_blank}.
...
Here's the code that we used to make the plot. Notice that it contains three functions: `ggplot()`, `aes()`, and `geom_point()`.
...
`aes()` is used to gather together all of the aesthetics mappings that you want to create. It defines which variables in your dataset are mapped to which axes in your graph.
...
`geom_point()` adds a layer of points to the empty plot created by `ggplot() + aes()`. This gives us a scatterplot. 
...
2. Select variables with the `aes()` function

3. Add elements to the graph with a `geom_` function
...
Give it a try! Replace the bracketed sections with `mpg`, `x = class, y = hwy`, and `geom_boxplot` to make a slightly different graph. Be sure to delete the `#` symbols before you run the code.
...
```{r blank-check}
"Good job! A ggplot that has no layers looks blank. To finish the graph, add an aes() contente and a geom function."
```
...
To map an aesthetic to a variable, set the name of the aesthetic equal to the name of the variable, and do this inside `aes()` function. ggplot2 will automatically assign a unique level of the aesthetic (here a unique color) to each unique value of the variable. ggplot2 will also add a legend that explains which levels correspond to which values.

This insight gives us a new way to think about the `aes()` function. It tells ggplot2 more than which variables to put on which axes, they tell ggplot2 which variables to map to which visual properties. The x and y locations of each point are just two of the many visual properties displayed by a point.
...
Until then, the best way to get a comprehensive overview of the available geoms is with the [ggplot2 cheatsheet](https://www.rstudio.com/wp-content/uploads/2016/11/ggplot2-cheatsheet-2.1.pdf){target="_blank}. To learn more about any single geom, look at its help page, e.g. `?geom_smooth`.
...
![](https://vimeo.com/223812632){target="_blank}
