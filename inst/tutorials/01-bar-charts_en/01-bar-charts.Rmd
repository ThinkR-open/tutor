...
As you saw in the code examples, you use `facet_grid()` by passing it variables names in `rows` and `cols`. 
...
If you do not wish to split on the vertical or horizontal dimension, use only `rows` or `cols`.
...
To use `facet_wrap()` pass it a single variable name in `facets`, contained in `vars()`.
