...
Ces exemples sont issus du livre _R for Data Science_ d'Hadley Wickham et Garrett Grolemund, publié par O’Reilly Media, Inc., 2016, ISBN: 9781491910399. Vous pouvez vous procurer le livre ici : [shop.oreilly.com](http://shop.oreilly.com/product/0636920034407.do){target="_blank}.

Ce module utilise le [packages qui composent le coeur du tidyverse](http://tidyverse.org/){target="_blank}, notamment {ggplot2}. Ce package a déjà été installé au préalable.
...
question(
  "Qu'en pensez-vous : Les voitures avec de gros moteurs utilisent-elles plus de carburant que les voitures avec de petits moteurs ?", 
  type = "single",
  correct = "Super !",
  answer(
    "Les voitures avec un plus gros moteur utilisent _plus_ de carburant.", 
    correct = TRUE, 
    message = "Autrement dit, il existe une relation positive entre la taille du moteur et le rendement énergétique. Maintenant testons cette hypothèse avec des données."
  ),
  answer(
    "Les voitures avec un plus gros moteur utilisent _moins_ de carburant.", 
    correct = TRUE, 
    message = "Autrement dit, il existe une relation négative entre la taille du moteur et le rendement énergétique. Maintenant testons cette hypothèse avec des données."
  )
)
```
...
Jusque là, le meilleur moyen d'obtenir une vue d'ensemble compréhensible sur les geoms, c'est d'utiliser la [cheatsheet {ggplot2}](https://www.rstudio.com/wp-content/uploads/2016/11/ggplot2-cheatsheet-2.1.pdf){target="_blank}, cette petite page remplie d'astuces trèsutiles. Et pour en savoir plus sur un geom en particulier, regardez sa page d'aide (par ex : `?geom_violin`.)
...
Si vous voulez en savoir d'avantage sur l’installation des packages R (ou R ou l'IDE -- environnement de développement intégré-- RStudio), la vidéo [Installation](http://tutorials.shinyapps.io/00-setup){target="_blank}) vous aidera dans le processus de paramétrage de R sur votre ordinateur.
