...
tutorial_options(exercise.timelimit = 220, exercise.checker = checker)
...
Vous pouvez demander à R de convertir chaque nouvelle colonne en un type de données approprié en utilisant l'argument `names_transform` à l'appel `pivot_longer()`. Cet argument doit être renseigné sous la forme `names_transform = list(year = as.integer)`. R transformera le contenu de la colonne `year` en entiers (integer). Essayez-le dans le code ci-dessus !
...
```{r ex9-solution}
table4b %>% 
  pivot_longer(cols = -country, names_to = "year", values_to = "population", names_transform = list(year = as.integer))
```
